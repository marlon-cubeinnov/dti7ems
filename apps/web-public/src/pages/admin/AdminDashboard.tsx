...
export function AdminDashboardPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Total Users" value={iStats?.users?.total ?? 0} icon={Users} color="bg-blue-600" sub={`${iStats?.users?.active ?? 0} active`} />
        <StatCard label="Enterprises" value={iStats?.enterprises?.total ?? 0} icon={Building2} color="bg-emerald-600" sub={`${iStats?.enterprises?.verified ?? 0} verified`} />
        <StatCard label="Total Events" value={eStats?.events?.total ?? 0} icon={CalendarDays} color="bg-purple-600" sub={`${eStats?.events?.byStatus?.COMPLETED ?? 0} completed`} />
        <StatCard label="Registrations" value={eStats?.participations?.total ?? 0} icon={UserCheck} color="bg-orange-500" sub={`${eStats?.participations?.recentRegistrations ?? 0} last 30d`} />
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Certificates Issued" value={eStats?.certificates?.issued ?? 0} icon={Award} color="bg-teal-600" sub={`of ${eStats?.certificates?.total ?? 0} total`} />
        <StatCard label="CSF Response Rate" value={`${eStats?.surveys?.csfResponseRate ?? 0}%`} icon={ClipboardCheck} color="bg-indigo-600" sub={`${eStats?.surveys?.submittedCsf ?? 0} submitted`} />
        <StatCard label="Attendance Records" value={eStats?.attendance?.totalRecords ?? 0} icon={TrendingUp} color="bg-cyan-600" />
        <StatCard label="Pending Users" value={iStats?.users?.pending ?? 0} icon={Clock} color="bg-amber-500" sub={`${iStats?.users?.suspended ?? 0} suspended`} />
      </div>
