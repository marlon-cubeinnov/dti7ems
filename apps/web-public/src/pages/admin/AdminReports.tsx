...
import { BarChart3, FileText, TrendingUp, Shield, Zap, Building2 } from 'lucide-react';
import { format } from 'date-fns';

type ReportTab = 'csf' | 'impact' | 'completion' | 'enterprise' | 'trends' | 'dpa';
...
const TABS: { key: ReportTab; label: string; icon: React.ElementType }[] = [
  ...
  { key: 'enterprise', label: 'Enterprise Training',   icon: Building2 },
...
export function AdminReportsPage() {
  ...
  return (
    <div className="space-y-4">
      ...
      {activeTab === 'enterprise' && <EnterpriseTrainingReport />}
...
function CsfReport() {
  const { data, isLoading } = useQuery({ queryKey: ['admin-report-csf'], queryFn: () => adminEventsApi.getCsfReport() });
  ...
  if (!report || report.count === 0) return <div className="bg-white rounded-xl shadow-card p-8 text-center text-gray-400">No CSF survey responses yet.</div>;
...
function ImpactReport() {
  const { data, isLoading } = useQuery({ queryKey: ['admin-report-impact'], queryFn: () => adminEventsApi.getImpactReport() });
  const report = (data as any)?.data;
  if (isLoading) return <div className="bg-white rounded-xl shadow-card p-8 text-center text-gray-400">Loading…</div>;
  if (!report || report.count === 0) return <div className="bg-white rounded-xl shadow-card p-8 text-center text-gray-400">No impact survey responses yet.</div>;
...
function CompletionReport() {
  const { data, isLoading } = useQuery({ queryKey: ['admin-report-completion'], queryFn: () => adminEventsApi.getCompletionReport() });
  const events = Array.isArray((data as any)?.data) ? (data as any).data : [];
  if (isLoading) return <div className="bg-white rounded-xl shadow-card p-8 text-center text-gray-400">Loading…</div>;
  if (events.length === 0) return <div className="bg-white rounded-xl shadow-card p-8 text-center text-gray-400">No completed events yet.</div>;
          ...
          <tbody className="divide-y divide-gray-100">
            {events.map((ev: any) => (
              <tr key={ev.id}>
                ...
                <td className="px-4 py-2 text-xs text-gray-500 whitespace-nowrap">{format(new Date(ev.endDate), 'MMM d, yyyy')}</td>
                ...
                <td className="px-4 py-2 text-right">
                  <span className={ev.attendanceRate >= 70 ? 'text-green-600' : 'text-amber-600'}>{ev.attendanceRate}%</span>
                </td>
                <td className="px-4 py-2 text-right">{ev.certified}</td>
                <td className="px-4 py-2 text-right">
                  <span className={ev.csfResponseRate >= 50 ? 'text-green-600' : 'text-amber-600'}>{ev.csfResponseRate}%</span>
                </td>
              </tr>
            ))}
          </tbody>
...
function TrendsReport() {
  const { data, isLoading } = useQuery({ queryKey: ['admin-report-trends'], queryFn: () => adminEventsApi.getTrends(12) });
  const trends = Array.isArray((data as any)?.data) ? (data as any).data : [];
  const maxVal = Math.max(...trends.map((t: any) => t.registrations), 1);
  if (isLoading) return <div className="bg-white rounded-xl shadow-card p-8 text-center text-gray-400">Loading…</div>;
  if (trends.length === 0) return <div className="bg-white rounded-xl shadow-card p-8 text-center text-gray-400">No registration data yet.</div>;
      ...
      <div className="flex items-end gap-2 h-48">
        {trends.map((t: any) => (
          <div key={t.month} className="flex-1 flex flex-col items-center gap-1">
            ...
            <div className="w-full bg-dti-blue rounded-t-sm transition-all" style={{ height: `${(t.registrations / maxVal) * 100}%`, minHeight: '4px' }} />
            <span className="text-[10px] text-gray-400 -rotate-45 origin-top-left whitespace-nowrap">{t.month}</span>
          </div>
        ))}
      </div>
    </div>
...
function DpaReport() {
  const { data, isLoading } = useQuery({ queryKey: ['admin-report-dpa'], queryFn: () => adminEventsApi.getDpaReport() });
  const report = (data as any)?.data;
  if (isLoading) return <div className="bg-white rounded-xl shadow-card p-8 text-center text-gray-400">Loading…</div>;
...
function EnterpriseTrainingReport() {
  const { data, isLoading } = useQuery({
    queryKey: ['admin-report-enterprise-training'],
    queryFn: () => adminEventsApi.getEnterpriseTrainingReport(),
  });
  const report = (data as any)?.data;

  if (isLoading) return <div className="bg-white rounded-xl shadow-card p-8 text-center text-gray-400">Loading…</div>;
  if (!report || report.summary.totalEnterprises === 0) {
    return <div className="bg-white rounded-xl shadow-card p-8 text-center text-gray-400">No enterprise training data yet. Enterprises must be linked to user accounts and their members must attend events.</div>;
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="bg-white rounded-xl shadow-card p-5">
          <p className="text-xs text-gray-500 uppercase tracking-wide">Enterprises with Training</p>
          <p className="text-3xl font-bold text-gray-900 mt-1">{report.summary.totalEnterprises}</p>
          <p className="text-xs text-gray-400 mt-1">Companies with at least one member who attended training</p>
        </div>
        <div className="bg-white rounded-xl shadow-card p-5">
          <p className="text-xs text-gray-500 uppercase tracking-wide">Total Training Completions</p>
          <p className="text-3xl font-bold text-gray-900 mt-1">{report.summary.totalTrainingCompletions}</p>
          <p className="text-xs text-gray-400 mt-1">Enterprise × training event combinations completed</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-card overflow-hidden">
        <div className="p-5 border-b">
          <h3 className="text-sm font-semibold text-gray-900">Enterprise Training Completion Detail</h3>
          <p className="text-xs text-gray-400 mt-1">Shows which companies completed training and which members attended on their behalf</p>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 border-b">
                <th className="text-left px-4 py-3 font-medium text-gray-600">Enterprise</th>
                <th className="text-left px-4 py-3 font-medium text-gray-600">Training / Event</th>
                <th className="text-left px-4 py-3 font-medium text-gray-600">Date</th>
                <th className="text-center px-4 py-3 font-medium text-gray-600">Members</th>
                <th className="text-left px-4 py-3 font-medium text-gray-600">Attended By</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {report.enterprises.map((ent: any) =>
                ent.events.map((evt: any, idx: number) => (
                  <tr key={`${ent.enterpriseId}-${evt.eventId}`} className="hover:bg-gray-50">
                    {idx === 0 ? (
                      <td className="px-4 py-3 font-medium text-gray-900 align-top" rowSpan={ent.events.length}>
                        <div>{ent.enterpriseName ?? '—'}</div>
                        <div className="text-xs text-gray-400 mt-0.5">{ent.totalMembersParticipated} member(s) participated</div>
                      </td>
                    ) : null}
                    <td className="px-4 py-3 text-gray-700">{evt.eventTitle}</td>
                    ...
                    <td className="px-4 py-3 text-xs text-gray-500">
                      {evt.members.map((m: any) => m.name || m.email || '—').join(', ')}
                    </td>
                  </tr>
                ))
              )}
